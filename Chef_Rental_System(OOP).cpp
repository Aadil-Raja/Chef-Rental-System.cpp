...
public:
 ...
 void get_chef_details(int x)
 {
  ...
  int view(int flag, char x[30], char y[20]) 
  {
    ...
    else 
	{
      ...
      while (!fin.eof())
	  {
		...
		{
          if (strcmp(cuisine, x) == 0 && strcmp(city, y) == 0 && availability == true) //to view chefs depending on user city and cusine which was seleted
		  {
            get_chef_details(0);
            ++c;
          }
  ...
  bool view2(int x, int y)       //for login
  {
    ifstream fin;
    int flag = 0;
    fin.open("CHEF_signup100.txt", ios::in);
